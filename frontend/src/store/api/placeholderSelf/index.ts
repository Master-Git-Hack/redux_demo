...
export const placeholderSelfApi = createApi({
	reducerPath: "PlaceholderSelfApi",
	...
	endpoints: (builder) => ({
        ...
        getUsers: builder.query<IUser[],null>({
            ...
            transformResponse: ({ data, ...response }: any) => ({
                data: data?.map(({ email, ...rest }: IUser) => {
...
//export const getPlaceholcerSelfAPI = (state: RootState) => state.PlaceholderSelfApi;